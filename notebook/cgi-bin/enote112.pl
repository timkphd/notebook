...
#
#  Copyright (C) 1996-2004  Oak Ridge National Laboratory
#
#  Al Geist 
#  Noel M. Nachtigal
#  David Jung
...
#    Pasted plain-text now displays autowrapped at 80 characters
#    File input type 'Auto-Detect' option available.
#    Minor bug fixes
#
#  Version 1.11
...
#  If 0, uses the Sendmail.pm package which uses the SMTP server specified below
$main::useSendmailCommand = 0;

#  SMTP service host to use for outgoing email
# (required only if $main::useSendmailCommand is 0)
$main::SMTPHost = 'Petra';

#  Program to use for sending e-mail from enote server
# (required only if $main::useSendmailCommand is 1)
...
# Enable an 'auto-detect' opton for the file type during page add
#  0 = no auto-detect option, user selected type is used; 
#  1 = auto-detect option added, and when selected the 
#  browser trys to determine uploaded file type
$main::autodetecttype = 1;
