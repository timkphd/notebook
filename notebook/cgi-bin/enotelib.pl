...
$main::version = "EN 1.12d";
$main::copyright = "Copyright (C) 1996-2004  Oak Ridge National Laboratory";
#  version EN 1.12
...
#  c-changes July 18, 2003
#    added WRAP=HARD to the three <textarea> fields
#  d-changes Feb 4 2004
#    added Auto-Detect of MIME-type for uploaded file
#    Enforce user MIME-type choice even if the browser thinks it knows better 
#    (fixing problem with IE misinterpreting some files as octet-streams) 
#    bug fix for email addresses including '-' being considered invalid.
#    fixed bug where the left&right sides of a button were rendered even if the
#    button wasn't (e.g if it was disabled)
#  e-changes July 27 2004
#    changed WRAP=hard to WRAP=soft to avoid breaking long URLs midline
#    changed ADD and EDIT pages text on Caption
#    fixed typos in email notify message
#  f-changes October 10 2004
#    fixed image upload detect to display png images inline
#  
#
#  version EN 1.11
...
# Add this for wrapping plain text on 80 characters (gave errors so commented out)
...
EOF
  ...
  # the $main::showdeleted var used to be in enote.pl v1.12c - but is no
  # longer officially supported there.  However, for users that
  # were using the functionality, they can have it by setting
  # $main::hidedeleted = 1

  if ($main::hidedeleted == 0) {
    $main::showdeleted = 1;
  }

  if ($main::hidedeleted == 1) {
    $main::showdeleted = 0;
  }
...
main::in{'mimetype'} = $main::in{'mimetype'}<br>
main::up{'upfile'}{'mime'} = $main::up{'upfile'}{'mime'}<br>
...
#  It might be incomplete.
  elsif ( $author && $body && $mimetype && $title && ($valid == 1) ) {	# Complete, save it
...
<p><a href="$main::script?nb=$main::nb&action=view&page=last">
Click here to display your new entry</a></p>
EOF
      exit $main::RET_NO_ERROR;
    }
    else {
  ...
  else {					# Incomplete/invalid entry, retry
    $body = '' if ( ! $isentry );
    if ( "$mimetype" ne '' ) {
...
<textarea rows=15 cols=75 WRAP=SOFT name=body>$body</textarea><br>
</p>
...
You can <b>type</b> or <b>cut and paste</b> text or HTML 
into the text area (it will scroll to accommodate the input):<br>
<textarea rows=5 cols=75 WRAP=SOFT name=text>$text</textarea><br>
</p>
...
###############################################################################
#
#  ( @pagemap ) = &generatePageMap( );
#
# This creates an array of page numbers, ommiting deleted pages *if*
#  $main::showdeleted==0.  If there are no deleted pages then the array
#  will just be a 1-1 map (i.e. mapemap[n]==n )
# The array length returned is the number of visible pages, 
#  and the prev/next of any given page can be found by looking at the 
#  prev/next index in the array (after locating the index of the page no. 
#  in question).
# If $main::showdeleted==1, any empty array is returned (as there is no
#  need for a pagemap if the mapping is always 1-1)
# NB: objects are not considered.
#
###############################################################################
...
#  Open the index file and parse it.
  ...
  while ( <IDX> ) {
    ...
    if ( $tag > 0 ) {			# Page
      $pageno++;
      if (($name ne '') || ($main::showdeleted == 1)) {
	@pagemap = ( @pagemap, $pageno );
      }
    }
...
sub ActionView {
  my @pagemap = &generatePageMap(); #get array of visible pages (if we're hiding deleted pages)

  my ( $filename, $filetype, $objs, $pageno, $pages ) = ( '', '', 0, 0, 0);
  my ( $nextpageindex, $pageindex) = (0,0);
  my ( $nextpageno, $prevpageno) = (0,0);
  my  $flag = 0 ;
#  Open the index file and parse it.
  open( IDX, "<$main::idxfile" ) || &InvalidFile($main::idxfile);
  flock( IDX, $main::LOCK_EX ) if ( $main::flock );
  ...
  while ( <IDX> ) {
	...
	$filetype = lc( $type );
      ...
      if ($main::showdeleted == 0) {
        if ($pagemap[$nextpageindex] == $pages) {
	  $pageindex = $nextpageindex;
          $nextpageindex = $nextpageindex+1;
        }
      }
      else {
        $pagemap[$pages-1] = $pages;
      }


      if (("$main::page" eq 'first') && ($pages == $pagemap[0])) {
	$pageno     = $pages;
	$nextpageno = $pagemap[1];
	$prevpageno = 0;
	$main::in{'page'} = $pageno;
        $filename   = $name;
	$filetype   = lc( $type );
      ...
      elsif (("$main::page" eq 'last') && ($pages == $pagemap[-1])) {
	$pageno   = $pages;
	$nextpageno = 0;
	$prevpageno = $pagemap[@pagemap-2];
	$main::in{'page'} = $pageno;
        $filename   = $name;
        $filetype = lc( $type );
        $flag = $as_is ;
      }
      elsif ("$tag" eq "$main::page") {  # Found a matching tag
	$pageno     = $pages;
	$nextpageno = $pagemap[$nextpageindex];
	$prevpageno = $pagemap[$pageindex-1];
	if ($pageno == $pagemap[0]) { $prevpageno = 0; }
	if ($pageno == $pagemap[-1]) { $nextpageno = 0; }
        $filename   = $name;
	$filetype   = lc( $type );
        $flag = $as_is ;
      }
...
EOF
  ...
  $main::nextpage = $nextpageno;
  $main::prevpage = $prevpageno;
...
#  If prevpage valid, put a 'Prev' link.
  if ( $prevpageno > 0 ) {
...
#  If nextpage valid, put a 'Next' link.
  if ( $nextpageno > 0 ) {
...
#  If not last page, put a 'Last' link.
  if ( $pageno != $pagemap[-1] ) {
...
EOF
	    ...
	    if ("$method" eq 'subscribe') {
		print "<p><b>Subscription failed.  User <i>$name</i> may already be in the subscription list for this notebook, or the email address was invalid.</b></p>";
	    }
	    else {
...
#          'last'  : view last page
#          'next'  : view next page (which is $main::nextpage)
#          'prev'  : view previous page (which is $main::prevpage)
#
...
EOF
    ...
    my $button = '';
    foreach $button ( @buttonlist ) { # for each button 

      # check if we shouldn't render the button for some reason
      my $renderbutton = 1;
      if ( ("$button" eq "subscribe") && ( $main::subscribe == 0 )) {
          $renderbutton = 0;
      }
...
EOF
	    ...
	    print <<EOF;
<a
 href="$main::script?nb=$main::nb&action=view&page=first"
...
#   next if ( $tag < 0 );		# Skip notebook objects
	...
	if ( $main::showdeleted == 1 ) {
	    $text .= "<tr><td>$tag.</td><td align=left>Deleted page</td><td></td>\n";
	}
    }
    elsif ( $tag > 0) {
...
#  Scan the file for the author name, creation date, and title.
	...
	if    ( m/^<!-- Author: (.*) -->$/ ) { $author = $1; }
      ...
      close( FILE );
      $text .= "<tr><td>$tag.</td><td aligh=left><a href=\"$main::relscript?nb=$main::nb&action=view&" .
	       "page=$tag\">$title ($author)</a></td>\n";
      $text .= "<td align=right><nobr>[Last Modified $date]</nobr></td></tr>\n";
    }
  }
  ...
  my $deletedhiddentext = "";
  if ($main::showdeleted==0) {
    $deletedhiddentext = "<p><i><small>(page numbers of deleted pages not shown)</small></i></p>";
  }

  my $pagetext = <<EOF;
<table cellpadding=0>$text</table>
$deletedhiddentext
<hr>
...
sub outputSubscriptions
{
	...
	if ($main::showemails == 1) {
	    print "<i><small>(if you prefer not to have email addresses shown here, ask the notebook administrator to";
	    print " add the line \$main::showemails = 0 to the enote.pl configuration file.)</small></i>\n";
	}
...
<hr>
<p>You have received this e-mail because you are subscribed to E-Note update
notifications.  If you no longer wish to receive updates, go to the appropriate notebook
...
#  $html = &isHTML($text);
#
#  determines if the input $text is HTML (returns 1) or just text (returns 0)
#
###############################################################################
...
sub isHTML {
  ...
  # just look for a few typical tags for now
  if (   ((index $text, "</p>") != -1) 
      || ((index $text, "</P>") != -1)
      || ((index $text, "<br>") != -1) 
      || ((index $text, "<BR>") != -1) 
